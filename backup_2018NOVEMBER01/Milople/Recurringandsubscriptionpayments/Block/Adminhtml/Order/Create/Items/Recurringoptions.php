...
class Recurringoptions extends \Magento\Framework\View\Element\Template
{
	...
	*/
	public function getCalendarHtml($_item){
		$html = '<input type="text" class="input-text required-entry admin_subsc_start_date" id="milople_subscription_start_date_'.$_item->getProduct()->getId().'" name="milople_subscription_start_date_'.$_item->getProduct()->getId().'" aria-required="true" >';
		$html .=
		'<script>
			 require([
				  "jquery",
				 ...
				 $("#milople_subscription_start_date_'.$_item->getProduct()->getId().'").calendar({
